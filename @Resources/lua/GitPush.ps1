...
$repoRoot = $RepoPath
"Script started at $(Get-Date) for $repoRoot" | Out-File -FilePath "$repoRoot\debug.log" -Append
...
function Write-RepoNavReport {
  ...
  foreach ($nm in $keyNames) {
    ...
    if ($f) {
      ...
      $foundKeyFiles += [pscustomobject]@{ Name = $nm; RelPath = $rel; FullPath = $f.FullPath }
    }
  }
